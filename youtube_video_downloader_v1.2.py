...
def set_video_quality(quality):
    ...
    selected_video_quality.set(quality)  # Update the StringVar with the selected quality
...
def download_video():
    ...
    try:
        ...
        available_streams = youtube_object.streams.filter(file_extension='mp4').all()  # Get all available streams
        print("Available Streams:")
        for stream in available_streams:
            print(stream)

        # Filter available streams based on resolution and progressive download
        filtered_streams = youtube_object.streams.filter(res=selected_video_quality.get(), file_extension='mp4', progressive=True).all()

        if filtered_streams:  # Check if filtered streams exist
            stream = filtered_streams[0]  # Select the first stream
            output_path = download_location_var.get()
            stream.download(output_path)
            status_var.set("YouTube video downloaded successfully!")
        else:
            status_var.set("No available streams found.")
    except Exception as e:
